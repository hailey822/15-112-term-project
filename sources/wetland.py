...
### Wetland : User Interaction to create polygon Food generator and Death Trap
### This class works as wrapper class to hold both food generator and death trap
...
class WetLand(object):
    ...
    # If polygon is made, add this wetland
    def update(self):
    ...
    # Add new point to shape wetland
    def add(self, x, y):
        self.currentPoints.append((x,y))
    
    # Wetland disappears after certain time 
    def age(self):
        index = 0 
        newAges = []
        newTypes = [] 
    ...
    # Checks if the point is inside the wetland
    ...
    # Unique behavior of death trap  : kill entities entered into this polygon
    ...
    # Unique behavior of food generator : generate food based on the size of the polygon
    def generateFood(self, data):
        for indexL in range(len(self.lands)):
            ...
            for point in points: 
                food = Food(point.x, point.y)
                food.zone = data.gs.add( food, point.x, point.y)
                data.foods.append( food )
            
    # Generate random point in the polygon
